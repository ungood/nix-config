...
This configuration uses **Lix** (a community fork of Nix). The Lix module is imported automatically in the NixOS base configuration.
...
- NixOS and Home Manager configurations are auto-discovered from `configurations/` directory
...
### Module Arguments
- **NixOS/Home modules**: Receive `inputs` and `self` as special arguments via `specialArgs`/`extraSpecialArgs`
...
- Run configuration checks (`just test`) and builds (`just build` for NixOS)
